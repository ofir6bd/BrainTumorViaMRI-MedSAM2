...
1. **Patient split** — the split is **physical**: `training_data_additional/` holds `train/`, `val/` and `test/` subfolders of patient dirs — **1621 patients** total, **train 973 (60%) / val 324 (20%) / test 324 (20%)**, every one with all 4 MRI modalities + an expert `-seg` mask. [pipeline.py](pipeline.py) just reads them (`split_dirs` / `split_patients`); it does not shuffle or re-derive the split. All three splits are labeled; `test` is never trained on — only used for the post-training Dice summary and the web UI's patient list.
   ...
   - Because the assignment lives in the filesystem, it is reproducible by construction and stable across runs — nothing can silently reshuffle it. To change it, move a patient folder. `split_manifest.json` next to the three folders records the original assignment (made with `seed=42`, matching the split used by the existing weights).
...
| `seed` | The seed used when `fraction=` asks for a subset of each split (e.g. train on half the data): it picks a reproducible *random* sample rather than an alphabetical prefix. It no longer affects the train/val/test assignment itself — that is physical, one folder per split. | 42 |

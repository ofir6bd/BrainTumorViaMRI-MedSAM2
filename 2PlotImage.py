...
# BraTS 2024 Color Mapping: 0:Background, 1:NETC(Red), 2:SNFH(Green), 3:ET(Blue), 4:RC(Yellow)
BRATS_COLORS = ['none', 'red', 'green', 'blue', 'yellow']
BRATS_CMAP = ListedColormap(BRATS_COLORS)

def find_patients_in_training_data():
    """Find all patients with brain and segmentation files in training_data folder"""
    ...
    # Walk through all subdirectories in training_data
    for root, dirs, files in os.walk(dataset_dir):
        # Group files by directory (patient)
        nii_files = [f for f in files if f.endswith(('.nii', '.nii.gz'))]
        
        if len(nii_files) >= 2:  # Need at least brain + seg files
            brain_files = []
            seg_files = []
            ...
            # If we have both brain and segmentation files
            if brain_files and seg_files:
                patient_id = os.path.basename(root)
                
                # Skip if patient_id is just "training_data" (root folder)
                if patient_id == "training_data":
                    continue
                
                # Add all brain files for this patient
                for brain_file in brain_files:
                    seg_file = seg_files[0]  # Use first seg file
...
def plot_3d_scatter(seg_data, title="3D Segmentation Scatter", max_points=3000):
    """Create 3D scatter plot showing all available segments using BraTS colors"""
    try:
        print("Creating 3D scatter plot...")
        
        # Get unique segmentation values (excluding 0)
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        # FIXED: single set_title — removed duplicate that said "Jet Colormap"
...
def plot_patient(patient_info):
    """Plot brain and segmentation for a patient"""
    try:
        ...
        # Find best slice (most segmentation)
            ...
            try:
                ...
                if 0 <= num < min(len(patients), 100):
                    ...
                    if choice == '1':
                        ...
                        # Load segmentation data and create 3D scatter plot
